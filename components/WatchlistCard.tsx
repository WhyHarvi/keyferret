      ...
      <div className="relative overflow-hidden rounded-lg border border-border bg-surface transition-colors duration-300 hover:border-accent/40">
        ...
        >
          ...
          <button
            ...
            aria-label={tracking ? "Stop tracking this price" : "Track this price"}
            aria-pressed={tracking}
            className={`absolute left-2 top-2 flex h-6 w-6 items-center justify-center rounded-md backdrop-blur-sm transition-colors ${
              tracking ? "bg-accent text-white" : "bg-black/60 text-white/70 hover:text-white"
            }`}
          >
            {tracking ? <BellRing size={12} aria-hidden="true" /> : <Bell size={12} aria-hidden="true" />}
          </button>
