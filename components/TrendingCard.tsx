...
export default function TrendingCard({ game, rank }: TrendingCardProps) {
  const year = game.releaseDate ? game.releaseDate.slice(0, 4) : null;

  return (
    <Link href={`/game/${game.slug}`} scroll data-trending-card className="group block w-[140px] shrink-0 snap-start sm:w-[160px] md:w-[180px]">
      <div className="relative overflow-hidden rounded-lg border border-border bg-surface transition-colors duration-300 hover:border-accent/40">
        ...
        >
          ...
          <span className="absolute left-2 top-2 flex h-6 w-6 items-center justify-center rounded-md bg-black/60 text-xs font-bold text-white backdrop-blur-sm">
            {rank}
          </span>
